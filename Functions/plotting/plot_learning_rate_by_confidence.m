...
%% compute learning rate for each confidence value for each participant + average over all participants + compute stats for plot
params = modelling_outputs.parameters{model};    
data = stats_learning_rate_by_confidence_averaged_across_participants(params,model);

%% plot learning rate by confidence, averaged across all participants

% --- variables for figure settings
position_units = "centimeters";
%outer_position_size = [1, 1, 19, 15];
...
% --- plot
f = figure;
fill([data.confidence_rescaled fliplr(data.confidence_rescaled)],[data.CI_lower_confirmatory    fliplr(data.CI_upper_confirmatory)],  [color_dict.blue_autumn_palette] ,'EdgeColor','none','FaceAlpha',0.2) 
hold on 
fill([data.confidence_rescaled fliplr(data.confidence_rescaled)],[data.CI_lower_disconfirmatory fliplr(data.CI_upper_disconfirmatory)], [color_dict.red_autumn_palette] ,'EdgeColor','none','FaceAlpha',0.2)
hold on 
l1 = plot(data.confidence_rescaled, data.mean_LR_confirmatory, 'Color',[color_dict.blue_autumn_palette], 'LineWidth',my_line_width) ;
hold on
l2 = plot(data.confidence_rescaled, data.mean_LR_disconfirmatory, 'Color',[color_dict.red_autumn_palette], 'LineWidth',my_line_width);
hold on
%{
% overlay m1 learning rates on top
if add_inset_m1_confirmation_bias
    ...
    % --- plot 
    % Mean ± CI (outer, no marker)
    hCI_CON = errorbar(x, mean_CON, ci_CON, 'o', "MarkerSize",10, 'LineWidth',1.5, 'CapSize',18, "MarkerFaceColor",dark_blue, "MarkerEdgeColor",dark_blue, "Color",dark_blue);
    hCI_DIS = errorbar(x, mean_DIS, ci_DIS, 'o', "MarkerSize",10, 'LineWidth',1.5, 'CapSize',18, "MarkerFaceColor",dark_red, "MarkerEdgeColor",dark_red, "Color",dark_red);
...
%yticklabels(string(y_interval*100))
legend([l1, l2],'confirmatory','disconfirmatory')
title('Learning rate by confidence, averaged over participants (mean ± 95% CI)', 'FontName', 'Arial');
hold off 
% --- save figure
name = sprintf('Learning_rate_by_confidence_model%i_%s_outcomes_%s',model,outcome_encoding,version_name);
svnm = fullfile(figures_dir,name);
print(gcf,[svnm,'.svg'],'-dsvg')
print(gcf,[svnm,'.png'],'-dpng')
%saveas(gcf, [svnm,'.fig']);
clear subject_mean name svnm f1 pvalue1 pvalue2 pvalue3 pvalue4 pvalue5 pvalue6
